...
<div class="w" id="regist">
    <div class="mt">
        ...
        <div class="extra">
        <span>我已经注册，现在就&nbsp;
        	<a href="/taotao-sso/page/login" class="flk13">登录</a>
        </span>
        </div>
    </div>
    <div class="mc">
...
<script type="text/javascript">
	var REGISTER={
		...
		beforeSubmit:function() {
				//检查用户是否已经被占用
				$.ajax({
	            	url : REGISTER.param.surl + "/taotao-sso/user/check/"+escape($("#regName").val())+"/1?r=" + Math.random(),
	            	success : function(data) {
	            		if (data.data) {
	            			//检查手机号是否存在
	            			$.ajax({
	            				url : REGISTER.param.surl + "/taotao-sso/user/check/"+$("#phone").val()+"/2?r=" + Math.random(),
				            	success : function(data) {
				            		if (data.data) {
					            		REGISTER.doSubmit();
				            		} else {
		...
		login:function() {
			 location.href = "/taotao-sso/page/login";
			 return false;
		},
		reg:function() {
			if (this.inputcheck()) {
